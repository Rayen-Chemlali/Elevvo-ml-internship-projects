...
Mel spectrograms are rendered as small (3×3 inch) PNG images at 72 DPI and organised into `spectrograms_data/train/<genre>/` and `.../test/<genre>/` for use with Keras `ImageDataGenerator`. Already-existing images are skipped on re-runs.
...
2. **Lag features** in audio context translate to temporal statistics (MFCCs, chroma) — the 1-frame and aggregate statistics capture most genre-discriminative information.
3. **Classical and metal** are the easiest genres to classify — they occupy very distinct spectral regions.
4. **Rock and country** are frequently confused — both have similar rhythmic and timbral profiles.
5. **Transfer learning** generalises better than a CNN trained from scratch given only ~80 training images per genre.
6. **MFCC features dominate** feature importance in tree-based models — particularly MFCC 1 and 2 (energy and spectral tilt).
